...
class TilePicker(Frame):
    ...
    def handle_event(self, event):
        ...
        if self.hover and event.type == pygame.MOUSEMOTION:
            pos = Vector2(event.pos) - Vector2(self.global_rect.topleft)
            x = int(pos.x // self.app.level.tilemap.tileset.tile_size)
            y = int(pos.y // self.app.level.tilemap.tileset.tile_size)
            idx = int(y*self.rect.width//self.app.level.tilemap.tileset.tile_size + x)
            if 0 <= idx < len(self.app.level.tilemap.tileset.tiles):
                self.hovered = idx
            else:
                self.hovered = -1
            return True
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        # Draw tileset
        for idx, tile in enumerate(self.app.level.tilemap.tileset.tiles):
            x = (idx * self.app.level.tilemap.tileset.tile_size) % self.rect.width
            y = (idx * self.app.level.tilemap.tileset.tile_size) // self.rect.width * self.app.level.tilemap.tileset.tile_size
            surface.blit(TileRenderer.render(tile, [False]*8), (x, y))
            if idx == self.selected:
                pygame.draw.rect(
                    surface,
                    self.app.theme.colors["accent"],
                    (x, y, self.app.level.tilemap.tileset.tile_size, self.app.level.tilemap.tileset.tile_size),
                    2
                )
...
class MapCanvas(Frame):
    ...
    def __init__(self, parent: Optional[UIWidget], rect: Rect):
        ...
        self.logger = self.app.label_info
        width = self.app.level.tilemap.width * self.app.level.tilemap.tileset.tile_size
        height = self.app.level.tilemap.height * self.app.level.tilemap.tileset.tile_size
    ...
    def camera(self) -> Camera:
        """
        transform scaling attribut into a Camera object
        """
        return Camera(
    ...
    def fill(self, event: pygame.event.Event):
        ...
        x = int((mouse_pos.x - self.global_rect.left + self.scroll.x) // self.app.level.tilemap.tileset.tile_size)
        y = int((mouse_pos.y - self.global_rect.top + self.scroll.y) // self.app.level.tilemap.tileset.tile_size)
        if 0 <= x < self.app.level.tilemap.width and 0 <= y < self.app.level.tilemap.height:
            target_tile = self.app.level.tilemap.grid[y][x]
            ...
            while to_fill:
                ...
                if (cx, cy) in filled:
                    continue
                current_tile = self.app.level.tilemap.grid[cy][cx]
                if current_tile == target_tile:
                    self.app.level.tilemap.grid[cy][cx] = replacement_tile
                    filled.add((cx, cy))
                    # Add neighbors
                    if cx > 0:
                        to_fill.append((cx - 1, cy))
                    if cx < self.app.level.tilemap.width - 1:
                        to_fill.append((cx + 1, cy))
                    if cy > 0:
                        to_fill.append((cx, cy - 1))
                    if cy < self.app.level.tilemap.height - 1:
                        to_fill.append((cx, cy + 1))
    ...
    def handle_event(self, event: pygame.event.Event):
                    ...
                    if self.tile_picker.selected != -1:
                        for iy in range(int(y1), int(y2)+1):
                            for ix in range(int(x1), int(x2)+1):
                                self.app.level.tilemap.grid[iy][ix] = self.tile_picker.selected
        ...
        if self.painting:
            ...
            if 0 <= x < self.app.level.tilemap.width and 0 <= y < self.app.level.tilemap.height:
                if self.tile_picker.selected != -1:
                    self.app.level.tilemap.grid[y][x] = self.tile_picker.selected
        ...
        if self.erasing:
            ...
            y = int((mouse_pos.y - self.global_rect.top + self.scroll.y) // self.app.level.tilemap.tileset.tile_size)
            if 0 <= x < self.app.level.tilemap.width and 0 <= y < self.app.level.tilemap.height:
                self.app.level.tilemap.grid[y][x] = -1
    ...
    def render(self, surface):
        ...
        TilemapRenderer.render(self.app.level.tilemap, self.surface, self.camera)
        ...
        if self.estimating_rect and self.rect_start is not None:
            ...
            if 0 <= x < self.app.level.tilemap.width and 0 <= y < self.app.level.tilemap.height:
                ...
                pygame.draw.rect(
                    ...
                    self.app.theme.colors["accent"],
                    Rect(
                        Vector2(x1, y1) * self.app.level.tilemap.tileset.tile_size - Vector2(self.scroll) + Vector2(self.rect.topleft),
                        ((x2 - x1 + 1) * self.app.level.tilemap.tileset.tile_size, (y2 - y1 + 1) * self.app.level.tilemap.tileset.tile_size)
                    ),
                    2
                )
        else:
            mouse_pos = Vector2(pygame.mouse.get_pos())
            x = int(
                (mouse_pos.x - self.global_rect.left + self.scroll.x) // self.app.level.tilemap.tileset.tile_size
            )
            y = int(
                (mouse_pos.y - self.global_rect.top + self.scroll.y) // self.app.level.tilemap.tileset.tile_size
            )
            if 0 <= x < self.app.level.tilemap.width and 0 <= y < self.app.level.tilemap.height:
                pygame.draw.rect(
                    surface,
                    self.app.theme.colors["accent"],
                    Rect(
                        Vector2(x, y) * self.app.level.tilemap.tileset.tile_size - Vector2(self.scroll) + Vector2(self.rect.topleft),
                        (self.app.level.tilemap.tileset.tile_size, self.app.level.tilemap.tileset.tile_size)
                    ),
                    2
                )
...
class MiniMap(Frame):
    ...
    def update_minimap(self):
        ...
        TilemapRenderer.clear_cache()
        TilemapRenderer.render(self.app.level.tilemap, self._cache, self.camera)
...
class LevelEditor(UIApp):
    ...
    def __init__(self, size=(1280, 800)):
        ...
        self.running = True
        self.level = AssetsRegistry.load_level("test", Engine())
    ...
    def _setup_ui(self, size):
        ...
        # attach tilemap editor widgets to their tabs
        # Picker: TilePicker
        # Canvas: MapCanvas
        # PropertiesEditor: TilePropertiesEditor (to be implemented)
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.level.tilemap.tileset.update_animation(dt)
