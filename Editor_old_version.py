...
class MapCanvas(Frame):
    ...
    def reinit(self):
        ...
        self.estimating_rect: bool = False
        self.rect_start: Optional[Vector2] = None
        # Clear the renderer cache when changing tilemaps
        TilemapRenderer.clear_cache()

    @property
    def camera(self) -> Camera:
        # Create a camera that looks at the viewport starting from scroll position
        # Camera.pos is the CENTER, so we need to offset it by half the viewport size
        top_left = Vector2(self.scroll.x, self.scroll.y)
        center = top_left + Vector2(self.rect.width // 2, self.rect.height // 2)
    ...
    def render(self, surface):
        if not self.displayed:
            return
        tm = self.get_tilemap()
        # Create a full-size surface for the renderer to work with
        full_surface = pygame.Surface((tm.width * tm.tileset.tile_size, tm.height * tm.tileset.tile_size), pygame.SRCALPHA)
        TilemapRenderer.clear_cache()
        camera = self.camera  # Store camera in a variable to avoid creating multiple instances
        TilemapRenderer.render(tm, full_surface, camera, camera)
        
        # Extract the viewport portion and blit it
...
class MiniMap(Frame):
    ...
    def update_minimap(self):
        ...
        # Render the tilemap to a temporary surface at the tilemap's full size
        full_map_surface = pygame.Surface((map_w, map_h), pygame.SRCALPHA)
        TilemapRenderer.clear_cache()
        TilemapRenderer.render(self.app.level.tilemap, full_map_surface, self.camera, self.camera)
        self._cache = pygame.transform.smoothscale(full_map_surface, (new_w, new_h))
